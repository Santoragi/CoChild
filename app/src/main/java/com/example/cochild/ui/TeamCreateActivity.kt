...
/*
            // Firestore 팀 ID 생성
            teamId = firestore.collection("teams").document().id // 명시적으로 팀 ID 생성

            // 이미지가 선택되었는지 확인 후 업로드
            if (selectedImageUri != null) {
                uploadImageToStorage(selectedImageUri!!, teamName, teamCategory, teamDescription)
            } else {
                // 이미지가 없는 경우 바로 팀 생성
                createTeam(
                    profilePhoto = null,
...
//                updateUserTeamsField(userId, teamId, name) // user 테이블 수정
    ...
    private fun updateUserTeamsField(userId: String, teamId: String, teamName: String) {
        val teamInfo = mapOf("teamId" to teamId, "name" to teamName)
    ...
    private fun updateTeamInfo(name: String, category: String, description: String) {
        ...
        if (selectedImageUri != null) {
            // 새로운 이미지 업로드
            uploadImageToStorage(selectedImageUri!!) { imageUrl ->
                currentProfilePhotoUrl?.let { deleteImageFromStorage(it) } // 기존 이미지 삭제
                updates["profileImage"] = imageUrl // 새 이미지 URL 저장
                saveUpdates(updates) // Firestore 업데이트
            }
        } else {
            // 이미지 변경 없이 업데이트
            saveUpdates(updates)
    ...
    /*
    private fun uploadImageToStorage(imageUri: Uri, name: String, category: String, description: String) {
        val storageRef = storage.getReference("team_images/$teamId.jpg")

        storageRef.putFile(imageUri)
            .addOnSuccessListener {
                storageRef.downloadUrl.addOnSuccessListener { uri ->
                    uploadedImageUrl = uri.toString() // 업로드된 URL 저장
                    Toast.makeText(this, "이미지 업로드 성공", Toast.LENGTH_SHORT).show()
                    Log.d("TeamCreateActivity", "이미지 업로드 성공: $uploadedImageUrl")
                    createTeam(
                        profilePhoto = Uri.parse(uploadedImageUrl),
                        name = name,
                        category = category,
            ...
            .addOnFailureListener { e ->
                ...
                Log.e("TeamCreateActivity", "Firebase Storage 이미지 업로드 실패", e)
            }
    }
 */
