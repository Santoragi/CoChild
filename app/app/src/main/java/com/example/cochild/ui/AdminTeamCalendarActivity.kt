...
class AdminTeamCalendarActivity : AdminBaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val calendar = findViewById<CalendarView>(R.id.calendarView)    // 캘린더 객체
        val calendarDate = findViewById<TextView>(R.id.calendar_date)        // 캘린더의 선택된 날짜 텍스트 뷰
        val calendarInstance01 = Calendar.getInstance()         // SimpleDateFormat을 사용해 날짜를 원하는 형식으로 변환 및 초기날짜 세팅
...
//        Toast.makeText(this, "$filteredNotices", Toast.LENGTH_SHORT).show()
    ...
    private fun goIntoNotice(notices: List<Notice>) {
        ...
        var index = 0       // 현재 출력해줄 공지사항의 인덱스
        val noticesSize = notices.size
        val goIntoNoticeButtons = findViewById<LinearLayout>(R.id.notice_buttons_container)     // 공지사항 상세페이지로 이동하는 버튼
